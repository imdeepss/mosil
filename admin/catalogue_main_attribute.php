...
// Page title
$page_title = "Main Attribute";
$active_menu = "catalogue_main_attribute";


//fetch from DB
// SQL query to fetch data from main_attribute table
$sql = "SELECT * FROM main_attribute";
...
$fetch_sql = "SELECT id, parent_attr_name FROM parent_attribute";
$parent_attr_result = $conn->query($fetch_sql);

// Create an array to hold categories
$parent_attr_categories = [];

if ($parent_attr_result->num_rows > 0) {
    // Fetch each row as an associative array
    while ($row = $parent_attr_result->fetch_assoc()) {
        $parent_attr_categories[] = [
            'id' => (int) $row['id'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        $categoryName = sanitizeInput($_POST['main_attribute_name'] ?? '');
        $parentAttrId = (int) sanitizeInput($_POST['parent_attr'] ?? '');
        ...
        if ($action === 'add' || $action === 'edit') {
            ...
            if (empty($categoryName) || empty($parentAttrId) || empty($status)) {
                ...
                if ($action === 'add') {
                    // Insert new Main Attribute
                    $stmt = $conn->prepare("INSERT INTO main_attribute (main_attr_name, parent_attr, meta_title, meta_keywords, meta_description, status) VALUES (?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("ssssss", $categoryName, $parentAttrId, $metaTitle, $metaKeywords, $metaDescription, $status);
                    $stmt->execute();

                    $message = "Main Attribute added successfully.";
                    $messageType = "success";
                } else {
                    // Update existing Main Attribute
                    $stmt = $conn->prepare("UPDATE main_attribute SET main_attr_name = ?, parent_attr = ?, meta_title = ?, meta_keywords = ?, meta_description = ?, status = ? WHERE id = ?");
                    $stmt->bind_param("ssssssi", $categoryName, $parentAttrId, $metaTitle, $metaKeywords, $metaDescription, $status, $categoryId);
                    $stmt->execute();
                    $message = "Main Attribute updated successfully.";
                    $messageType = "success";
                }
            }
        } elseif ($action === 'delete') {
            // Delete Main Attribute
            $stmt = $conn->prepare("DELETE FROM main_attribute WHERE id = ?");
            $stmt->bind_param("i", $categoryId);
            $stmt->execute();

            $message = "Main Attribute deleted successfully.";
            $messageType = "success";
        } elseif ($action === 'publish' || $action === 'unpublish') {
            $newStatus = ($action === 'publish') ? 'Active' : 'Inactive';

            $stmt = $conn->prepare("UPDATE main_attribute SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?");
            $stmt->bind_param("si", $newStatus, $categoryId);
            $stmt->execute();

            $message = ($action === 'publish') ? "Main Attribute published successfully." : "Main Attribute unpublished successfully.";
            $messageType = "success";
        }
                                    ...
                                    <div class="modal-dialog modal-xl">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editCategoryModalLabel<?php echo $category['id']; ?>">Edit Main Attribute</h5>
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-6 mb-3">
                                                            <label for="parent_attr" class="form-label">Select Main Attribute</label>
                                                            <select class="form-select" id="parent_attr<?php echo $category['id']; ?>" name="parent_attr" required>
                                                                <option value="">-- Select Category --</option>
                                                                <?php foreach ($parent_attr_categories as $parent_attr): ?>
                                                                    <option value="<?php echo $parent_attr['id']; ?>"
                                                                        <?php echo ($parent_attr['id'] == $category['parent_attr']) ? 'selected' : ''; ?>>
                                                                        <?php echo htmlspecialchars($parent_attr['name']); ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        </div>
                                                        <div class="col-md-6 mb-3">
                                                            <label for="main_attribute_name<?php echo $category['id']; ?>" class="form-label">Main Attribute</label>
                                                            <input type="text" class="form-control" id="main_attribute_name<?php echo $category['id']; ?>" name="main_attribute_name" value="<?php echo htmlspecialchars($category['name']); ?>" required>
                                                        </div>
                                                        <div class="col-md-6 mb-3">
        ...
        <div class="modal-content">
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" class="needs-validation" enctype="multipart/form-data" id="addCategoryForm" novalidate>
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="parent_attr" class="form-label">Select Main Category</label>
                            <select class="form-select" id="parent_attr<?php echo $category['id']; ?>" name="parent_attr" required>
                                <option value="">-- Select Category --</option>
                                <?php foreach ($parent_attr_categories as $parent_attr): ?>
                                    <option value="<?php echo $parent_attr['id']; ?>">
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="main_attribute_name" class="form-label">Main Attribute Name</label>
                            <input type="text" class="form-control" id="main_attribute_name" name="main_attribute_name" required>
