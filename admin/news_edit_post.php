...
// Fetch News Post data
$post = null;
if ($postId > 0) {
    $postSql = "SELECT * FROM news_posts WHERE id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        header("Location: news_posts.php?error=Post not found");
        exit;
    }
} else {
    header("Location: news_posts.php?error=Invalid post ID");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($slug)) {
        ...
        $checkSlugSql = "SELECT id FROM news_posts WHERE slug = ? AND id != ?";
    ...
    if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExt, $allowedExtensions)) {
            $newImageName = 'News_' . time() . '_' . uniqid() . '.' . $fileExt;
    ...
    if (empty($errors)) {
        try {
            // Update database
            $updateSql = "UPDATE news_posts SET title = ?, slug = ?, short_description = ?, content = ?, image = ?, category_id = ?, tags = ?, status = ?, meta_title = ?, meta_description = ?, updated_at = NOW() WHERE id = ?";

            $stmt = $conn->prepare($updateSql);
            $stmt->bind_param("sssssissssi", $title, $slug, $shortDescription, $content, $imageName, $categoryId, $tags, $status, $metaTitle, $metaDescription, $postId);
            ...
            if ($stmt->execute()) {
                ...
                header("Location: news_edit_post.php?id=" . $postId . "&success=1");
                exit;
            } else {
                $errors[] = "Failed to update News Post. Please try again.";
            }
        } catch (Exception $e) {
    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h2">Edit News Post</h1>
                <div class="btn-toolbar mb-2 mb-md-0">
                    <div class="btn-group me-2">
                        <a href="news_posts.php" class="btn btn-sm btn-outline-secondary">
                            <i class="fas fa-arrow-left me-1"></i> Back to Posts
                        </a>
                        <a href="news_add_post.php" class="btn btn-sm btn-outline-primary">
            ...
            <div class="row">
                <div class="col-lg-8">
                    <form id="NewsPostForm" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . '?id=' . $postId; ?>" enctype="multipart/form-data" class="needs-validation" novalidate>
                        ...
                        <div class="card shadow-sm mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="short_description" name="short_description" rows="3" placeholder="Brief description of the News Post..."><?php echo htmlspecialchars($formData['short_description'] ?? ''); ?></textarea>
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="status" id="status_draft" value="Draft" <?php echo ($formData['status'] === 'Draft') ? 'checked' : ''; ?>>
                                        <label class="form-check-label" for="status_draft">
                                            <i class="fas fa-edit text-warning me-1"></i> Draft
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="status" id="status_published" value="Published" <?php echo ($formData['status'] === 'Published') ? 'checked' : ''; ?>>
                            ...
                            <div class="d-grid">
                                <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-save me-1"></i> Update News Post
                                </button>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            <?php if (!empty($post['image'])): ?>
                                <div class="mb-3">
                                    ...
                                    <div>
                                        <img src="../uploads/news/<?php echo htmlspecialchars($post['image']); ?>" alt="Current featured image" class="img-fluid rounded" style="max-height: 200px;">
                                    </div>
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="hidden" class="existing_image" value="../uploads/news/<?php echo htmlspecialchars($post['image']); ?>" />
                                <div class="form-text">Leave empty to keep current image. Recommended size: 1200x630px. Max file size: 5MB.</div>
                            </div>
...
<!-- Include TinyMCE -->
<script src="https://cdn.tiny.cloud/1/r5jlbs90sue98amwj0ur2zlo39grg8cbd8g0dypglitvqd3e/tinymce/7/tinymce.min.js" referrerpolicy="origin"></script>
...
<script>
    $(document).ready(function() {
        ...
        // Initial slug check for edit form
