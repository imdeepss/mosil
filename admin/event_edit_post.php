...
$categories = [];
$categorySql = "SELECT id, name FROM event_categories ORDER BY name";
...
function generateUniqueSlug($conn, $title, $excludeId = null)
{
    ...
    while (true) {
        $checkSql = "SELECT id FROM event_posts WHERE slug = ?";
        if ($excludeId) {
            $checkSql .= " AND id != ?";
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($slug)) {
        ...
        $checkSlugSql = "SELECT id FROM event_posts WHERE slug = ? AND id != ?";
    ...
    if (empty($errors)) {
        try {
            // Update database
            $updateSql = "UPDATE event_posts SET title = ?, slug = ?, short_description = ?, content = ?, image = ?, category_id = ?, tags = ?, status = ?, meta_title = ?, meta_description = ?, updated_at = NOW() WHERE id = ?";
                        ...
                        <div class="card shadow-sm mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="short_description" name="short_description" rows="3" placeholder="Brief description of the Event Post..."><?php echo htmlspecialchars($formData['short_description'] ?? ''); ?></textarea>
                                    <div class="form-text">This will be used as excerpt in Event Listings.</div>
                                </div>
                        ...
                        <div class="card-body">
                            <?php if (!empty($post['image'])): ?>
                                <div class="mb-3">
                                    ...
                                    <div>
                                        <img src="../uploads/events/<?php echo htmlspecialchars($post['image']); ?>" alt="Current featured image" class="img-fluid rounded" style="max-height: 200px;">
                                    </div>
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="hidden" class="existing_image" value="../uploads/events/<?php echo htmlspecialchars($post['image']); ?>" />
