...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    $stmt = $conn->prepare("SELECT image FROM blog_posts_v2 WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $res = $stmt->get_result();
    if ($post = $res->fetch_assoc()) {
        if (!empty($post['image']) && file_exists('../assets/uploads/blog/' . $post['image'])) {
            unlink('../assets/uploads/blog/' . $post['image']);
        }
    }
    
    $stmt = $conn->prepare("DELETE FROM blog_posts_v2 WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
