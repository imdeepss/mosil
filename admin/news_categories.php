...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        $name = $_POST['name'];
        $slug = $_POST['slug'];
        $stmt = $conn->prepare("INSERT INTO news_categories (name, slug) VALUES (?, ?)");
    ...
    if ($action === 'edit') {
        ...
        $name = $_POST['name'];
        $slug = $_POST['slug'];
        $stmt = $conn->prepare("UPDATE news_categories SET name = ?, slug = ? WHERE id = ?");
...
<div class="modal fade" id="addCategoryModal" tabindex="-1" aria-labelledby="addCategoryModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form method="post" action="" id="addCategoryForm" class="modal-content needs-validation" novalidate>
            <div class="modal-header">
                <h5 class="modal-title" id="addCategoryModalLabel">Add News Category</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
