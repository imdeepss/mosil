...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($slug)) {
        ...
        $checkSlugSql = "SELECT id FROM blog_posts_v2 WHERE slug = ?";
    ...
    if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExt, $allowedExtensions)) {
            $imageName = 'blog_' . time() . '_' . uniqid() . '.' . $fileExt;
    ...
    if (empty($errors)) {
        try {
            ...
            $insertSql = "INSERT INTO blog_posts_v2 (title, slug, short_description, content, image, category_id, tags, status, meta_title, meta_description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
            ...
            if ($stmt->execute()) {
                ...
                header("Location: blog_add_post.php?success=1");
                exit;
            } else {
