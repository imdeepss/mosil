...
$post = null;
if ($postId > 0) {
    $postSql = "SELECT * FROM blog_posts_v2 WHERE id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        header("Location: blog_posts.php?error=Post not found");
        exit;
    }
} else {
    header("Location: blog_posts.php?error=Invalid post ID");
    exit;
}

// Fetch blog categories
$categories = [];
$categorySql = "SELECT id, name FROM blog_categories ORDER BY name";
...
function generateUniqueSlug($conn, $title, $excludeId = null)
{
    ...
    while (true) {
        $checkSql = "SELECT id FROM blog_posts_v2 WHERE slug = ?";
        if ($excludeId) {
            $checkSql .= " AND id != ?";
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($slug)) {
        ...
        $checkSlugSql = "SELECT id FROM blog_posts_v2 WHERE slug = ? AND id != ?";
    ...
    if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExt, $allowedExtensions)) {
            $newImageName = 'blog_' . time() . '_' . uniqid() . '.' . $fileExt;
    ...
    if (empty($errors)) {
        try {
            // Update database
            $updateSql = "UPDATE blog_posts_v2 SET title = ?, slug = ?, short_description = ?, content = ?, image = ?, category_id = ?, tags = ?, status = ?, meta_title = ?, meta_description = ?, updated_at = NOW() WHERE id = ?";
            ...
            if ($stmt->execute()) {
                ...
                header("Location: blog_edit_post.php?id=" . $postId . "&success=1");
                exit;
            } else {
...
<div class="container-fluid">
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                ...
                <div class="btn-toolbar mb-2 mb-md-0">
                    <div class="btn-group me-2">
                        <a href="blog_posts.php" class="btn btn-sm btn-outline-secondary">
                            <i class="fas fa-arrow-left me-1"></i> Back to Posts
                        </a>
                        <a href="blog_add_post.php" class="btn btn-sm btn-outline-primary">
            ...
            <div class="row">
                <div class="col-lg-8">
                    <form id="blogPostForm" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . '?id=' . $postId; ?>" enctype="multipart/form-data" class="needs-validation" novalidate>
                        ...
                        <div class="card-body">
                            <?php if (!empty($post['image'])): ?>
                                <div class="mb-3">
                                    ...
                                    <div>
                                        <img src="../uploads/blog/<?php echo htmlspecialchars($post['image']); ?>" alt="Current featured image" class="img-fluid rounded" style="max-height: 200px;">
                                    </div>
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="hidden" class="existing_image" value="../uploads/blog/<?php echo htmlspecialchars($post['image']); ?>" />
