...
<section class="relative w-full h-[748px] md:h-[480px] overflow-hidden">
    <div class="absolute inset-0 z-0 w-full h-full">
        <img src="<?php echo SITE_URL; ?>/assets/images/banners/faq-banner.png" alt="Career"
            class="hidden md:block w-full h-full object-cover object-[50%_75%]" fetchpriority="high">

        <img src="<?php echo SITE_URL; ?>/assets/images/banners/faq-banner-mb.png" alt="Career"
    ...
    <div class="flex flex-col md:flex-row relative">
        ...
        <aside id="productCategorySidebar"
            ...
            <nav class="flex flex-col">
                <?php foreach ($categories as $index => $category):
                    ...
                    ?>
                    <label
                        class="flex items-center justify-between py-4 md:px-[60px] px-4 border-b border-[#FAE696] cursor-pointer group gap-4 hover:bg-[#FAE696] transition-colors">
                        <span class="text-[#1A3B1B] font-base font-normal text-[18px] leading-[140%] capitalize">
                            <?php echo $category; ?>
                        </span>
                        <input type="checkbox" <?php echo $isChecked; ?> class="faq-category-checkbox w-5 h-5 border border-[#1A3B1B] rounded-[4px] bg-transparent cursor-pointer appearance-none 
                          ...
                          checked:before:bg-[url('<?php echo SITE_URL; ?>/assets/icons/svg/check.svg')]"
        ...
        <main class="flex-1 px-4 py-6 md:px-[54px] md:py-7 bg-white">
            ...
            <nav
                ...
                <a href="<?php echo SITE_URL; ?>/newsroom" class="text-[#A3A3A3] font-light">
                    Newsroom
                </a>
            ...
            <div class="pt-[24px] mt-4 md:mt-0">
                <div class="border-b-2 border-primary md:pb-4 pb-2 flex items-center gap-4">
            ...
            <div id="faq-list-container" class="md:py-10 py-9.5">
...
<!-- Sidebar Overlay -->
<div id="sidebarOverlay" class="fixed inset-0 bg-black/50 z-50 hidden md:hidden"></div>

<script>
    const faqsData = <?php echo json_encode($faqs); ?>;

    document.addEventListener('DOMContentLoaded', function () {
        const categoryCheckboxes = document.querySelectorAll('.faq-category-checkbox');
        const listContainer = document.getElementById('faq-list-container');
        const titleElement = document.getElementById('faq-category-title');
        ...
        renderFaqs(Object.keys(faqsData)[0]);
        ...
        categoryCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', function () {
                if (this.checked) {
                    ...
                    // Prevent unchecking the active category (must have at least one selected)
                    this.checked = true;
                }
            });
        });
        ...
        function renderFaqs(category) {
            ...
            const items = faqsData[category];
            const html = items.map(item => `
                <li class="border-b-2 border-[#DEDEDE] last:border-0">
                    <h2 class="text-[#1A3B1B] font-base font-normal text-[18px] leading-[140%] pr-4 cursor-pointer faq-question w-full flex justify-between items-start gap-4">
                        ${item.question}
                        <span class="transform transition-transform duration-200 mt-1">
                            <!-- Optional Arrow Icon if Accordion desired, currently just static list implied but let's make it nice -->
                        </span>
                    </h2>
                    <p class="text-[#757575] font-base font-normal text-[16px] leading-[135%] tracking-[0.01em] py-3">
                        ${item.answer}
                    </p>
                </li>
            `).join('');
